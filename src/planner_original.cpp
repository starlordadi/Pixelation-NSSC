...
int white_thresh=200;//if BGR above this then treated as obstacle
...
int side_thresh=200;
float theta_thresh=20; // theta threshold between centroid of the bot and the waypoint/endpoint to consider as reached 
...
Localisation wptDetection( Mat img ,Point &c)
{
	    ...
	    R.x=S.y;
	    R.y=S.x;
	    DrawRotatedRectangle1(imgc, R ,Size(25,25),360-theta);   // Changes            
...
int main(int argc, char** argv)
{
    ...
    while(ros::ok())
    {
		...
		while (fabs(theta_error) > theta_thresh )              // thresholding
		{
			...
			cout<<"Position : "<<L.centroid<<" Theta : +
			;|"<<L.theta<<endl;
			if(theta_error>180) theta_error-=360;
		...
		// ros::Timer timer1 = nh.createTimer(ros::Duration(1.0), callback1);
        r.sleep();
        ros::spinOnce();
        // waitKey(1000);
    }
